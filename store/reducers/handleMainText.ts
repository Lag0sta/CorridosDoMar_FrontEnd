...
const handleMainTextSlice = createSlice({
  name: "handleMainText",
  initialState,
  reducers: {
    // Initialisation du Reducer
    initMainText: (state, action) => {
      state.value = action.payload
    },

    // Action 'add' avec un payload de type 'Item'
    addMainText: (state, action: PayloadAction<Item>) => {
    ...
    editMainText: (state, action: PayloadAction<{ text: string[]; type: string }>) => {
...
} = handleMainTextSlice.actions;
export default handleMainTextSlice.reducer;
